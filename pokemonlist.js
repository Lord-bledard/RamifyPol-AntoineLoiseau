const pokemonList = [
    ...
    "Goldeen", "Seaking", "Staryu", "Starmie", "Scyther", "Jynx", "Electabuzz", "Magmar", "Pinsir",
...
const dropdown2 = document.getElementById('pokemon-dropdown2');

pokemonList2.forEach(pokemon => {
    const option = document.createElement('option');
    option.value = pokemon;
    option.textContent = pokemon;
    if (dropdown2 != null)
...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    const pokemonDropdown2 = document.getElementById('pokemon-dropdown2');

    // Fetch Pokémon data from PokeAPI
    fetch('https://pokeapi.co/api/v2/pokemon?limit=151')  // Limiting to first 151 Pokémon (Gen 1)
        ...
        .then(data => {
            data.results.forEach(pokemon => {
                ...
                if (pokemonDropdown2) {
                    pokemonDropdown2.appendChild(option.cloneNode(true)); // For Versus Player page
                }
            });
        })
